...
filesystem/network/secret enforcement. Terminus wraps Pi in a strict
outer sandbox and supplies brokered network and secrets. Pi has no
native session resume (Terminus emulates it via checkpointing) and no
native compaction.
...
controls mean Terminus must compensate for every effect.
...
Terminus-full and Terminus-minimal isolates the contribution of the
