"""Deterministic grader for canary/edit-single-001 (single-file edit).

Success = full test suite (public + staged hidden boundary tests) passes and
only src/shipping.py changed. Repository-state and test-based only; no LLM
judge on the success path.
"""
...
def main() -> int:
    ...
    checks.append(("only src/shipping.py changed", changed == {"src/shipping.py"}, str(sorted(changed))))
