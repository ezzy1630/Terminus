"""Deterministic grader for canary/edit-multi-001 (multi-file edit).

Success = full test suite (public + staged hidden cross-file tests) passes,
all three pipeline files changed, and no unrelated tracked file changed.
...
EXPECTED_FILES = {"src/order.py", "src/receipt.py", "src/api.py"}
...
def main() -> int:
    ...
    checks.append(
        (
            "all three pipeline files changed and nothing else",
