/**
 * Terminus Desktop — Composer model picker.
 *
 * The composer routes a turn to one model. This is where that is chosen.
 *
 * Everything shown is reported by a provider at runtime — Terminus ships no
 * model list. When nothing is reported the picker says so and points at
 * Settings, because a plausible list the runtime cannot route to is worse than
 * an empty one.
 *
 * Two departures from the usual provider-switcher shape, both deliberate:
 *
 ...
 * to 1…9 while the picker is open. The digits are unmodified on purpose —
...
/** Search + filters + list + hint, used to decide whether the panel fits below. */
...
function ModelPickerImpl({ selection, className, disabled = false }: ModelPickerProps): JSX.Element {
  ...
  // Measure against the trigger at the moment the picker opens. The same
  // picker behaves correctly in the start composer (room below) and the
  // docked chat composer (none), without synchronously updating state from an
  // effect after every render.
  const openPicker = (event: React.MouseEvent<HTMLButtonElement>): void => {
  ...
  const selectedProvider = providerById(selected.provider);
  ...
  return (
    <div ref={rootRef} className={cn("relative", className)} onKeyDown={onKeyDown}>
      <Button
        ...
        data-tooltip="Model for this turn"
        onClick={(event) => {
          if (open) close();
          else openPicker(event);
        }}
        className={cn(
          "composer-control flex h-7 min-w-0 items-center gap-1.5 rounded-md border border-subtle bg-subtle px-2 text-xs text-secondary",
          "hover:text-primary disabled:cursor-not-allowed disabled:opacity-45",
          open && "border-default text-primary",
        )}
      >
        <ProviderMark mark={selectedProvider.mark} />
        <span className="max-w-36 truncate">{selected.label}</span>
      ...
      {open ? (
        ...
        >
          ...
          <div className="scrollable max-h-72 min-h-24 overflow-y-auto p-1">
            {rows.length === 0 ? (
              ...
              <section key={group.id} aria-label={group.title}>
                <h3 className="ui-section-label px-2 pb-1 pt-2">{group.title}</h3>
                {group.note ? (
                  <p className="px-2 pb-1.5 text-xs leading-snug text-tertiary">{group.note}</p>
                ) : null}
                  ...
                  return (
                    ...
                    >
                      ...
                      {model.id === selected.id ? (
                        <Check size={13} aria-label="Selected" className="shrink-0 text-accent" />
                      ) : slot >= 1 && slot <= 9 ? (
                        <span
                          aria-hidden
