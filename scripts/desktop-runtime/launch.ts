...
async function processTable(): Promise<readonly ProcessRow[]> {
  ...
  if (exitCode !== 0) throw new Error(`ps failed with exit ${exitCode}: ${stderr.slice(-4_000)}`);
...
function descendantPids(rootPid: number, rows: readonly ProcessRow[]): readonly number[] {
...
async function terminateOwnedProcessTree(child: ChildProcess): Promise<void> {
  ...
  for (const descendant of [...descendants].reverse()) signalIfRunning(descendant, "SIGTERM");
  await sleep(1_000);
  for (const descendant of [...descendants].reverse()) signalIfRunning(descendant, "SIGKILL");
}

async function privateMode(path: string, expectedMode: number, kind: "directory" | "file" | "socket"): Promise<void> {
  ...
  const expectedKind = kind === "directory"
    ...
    : kind === "file"
      ? details.isFile()
      : details.isSocket();
  if (!expectedKind || details.isSymbolicLink()) throw new Error(`${path} is not a private ${kind}`);
  if ((details.mode & 0o777) !== expectedMode) {
    throw new Error(`${path} mode is ${(details.mode & 0o777).toString(8)}, expected ${expectedMode.toString(8)}`);
  }
}
...
async function boundedLog(path: string): Promise<BoundedLogEvidence> {
  ...
  if (!details.isFile()) throw new Error(`desktop launch log is not a regular file: ${path}`);
  const retainedBytes = Math.min(details.size, MAX_RETAINED_LOG_BYTES);
  const handle = await open(path, "r");
  try {
    const bytes = Buffer.alloc(retainedBytes);
    if (retainedBytes > 0) await handle.read(bytes, 0, retainedBytes, details.size - retainedBytes);
    return {
...
async function verifyPublicRuntime(port: number, commit: string, version: string): Promise<void> {
  const headers = { Origin: "terminus://app" };
  const healthResponse = await fetch(`http://127.0.0.1:${port}/v1/system/health`, {
    headers,
    signal: AbortSignal.timeout(2_000),
  });
  const health = object(await healthResponse.json() as unknown, "public desktop health");
  const writer = object(health.writer, "public desktop health writer");
  if (
    healthResponse.status !== 200
    || healthResponse.headers.get("access-control-allow-origin") !== "terminus://app"
    || health.status !== "ok"
    || health.ready !== true
    ...
    throw new Error("packaged desktop public health identity/readiness is invalid");
  }
  const unauthenticated = await fetch(`http://127.0.0.1:${port}/v1/sessions?limit=1`, {
    headers,
    signal: AbortSignal.timeout(2_000),
  });
  if (
    unauthenticated.status !== 401
    || unauthenticated.headers.get("access-control-allow-origin") !== "terminus://app"
  ) {
    throw new Error("packaged desktop public API did not reject an unauthenticated request");
  }
}
...
async function artifactEvidence(path: string): Promise<ArtifactEvidence> {
  ...
  if (!details.isFile()) throw new Error(`desktop artifact is not a regular file: ${resolved}`);
  return { name: basename(resolved), sha256: await sha256File(resolved), size: details.size };
}

export async function launchDesktopRuntime(
  options: DesktopRuntimeLaunchOptions,
): Promise<DesktopRuntimeLaunchEvidence> {
  if (process.platform !== "darwin") throw new Error("desktop runtime launch evidence requires macOS");
  if (process.arch === "x64" && options.architecture === "arm64") {
    throw new Error("arm64 desktop launch evidence requires an Apple Silicon runner");
  }
  ...
  let completed = false;
  try {
    const boundArtifact = options.artifact === undefined
      ? undefined
      : await artifactEvidence(options.artifact);
    let application: string;
    if (options.artifact !== undefined) {
      if (!options.artifact.endsWith(".zip")) {
        throw new Error("exact desktop launch artifacts must be ZIP distributions");
      }
      const extracted = join(state, "exact-zip");
      await mkdir(extracted, { mode: 0o700 });
      await run(["/usr/bin/ditto", "-x", "-k", resolve(options.artifact), extracted]);
    ...
    if (
      ...
      throw new Error("packaged desktop identity does not match the requested launch evidence");
    }

    logHandle = await open(electronLog, "w", 0o600);
    const architectureFlag = options.architecture === "arm64" ? "-arm64" : "-x86_64";
    const launched = spawn(
      "/usr/bin/arch",
      [architectureFlag, executable, `--user-data-dir=${profile}`, "--no-first-run"],
      {
        cwd: state,
        env: {
          HOME: home,
    ...
    while (Date.now() - startedAt < STARTUP_TIMEOUT_MS) {
      if (!childRunning(launched)) {
        throw new Error(`packaged desktop exited before readiness\n${await readTail(electronLog)}\n${await readTail(runtimeLog)}`);
      }
      const runtimeText = await readTail(runtimeLog);
      const portMatch = /\[terminus-control\] listening on http:\/\/localhost:(\d+)/.exec(runtimeText);
      const rows = await processTable();
      const kernel = rows.some((row) => row.command === join(resources, "runtime", "bin", "terminus-kernel-mini"));
      const control = rows.some((row) => (
        row.command === `${join(resources, "runtime", "terminus-control", "bin", "terminus-control")} serve`
      ));
      const renderer = rows.some((row) => row.parentPid === launched.pid && row.command.includes("--type=renderer"));
    ...
    if (controlPort === 0) {
      throw new Error(`packaged desktop did not reach ready task surface\n${await readTail(electronLog)}\n${await readTail(runtimeLog)}`);
    }

    await verifyPublicRuntime(controlPort, options.commit, options.version);
    await sleep(HEALTH_STABILITY_MS);
    if (!childRunning(launched)) throw new Error("packaged desktop exited during the health-stability window");

    const electronText = await readTail(electronLog);
    const rendererCspLoaded = electronText.includes("[terminus-desktop] packaged renderer loaded under CSP");
    const rendererAuthInjected = electronText.includes("[terminus-desktop] authenticated header injected for renderer request");
    const rendererSseOpened = electronText.includes("[terminus-desktop] renderer opened SSE connection");
    if (!rendererCspLoaded) {
      throw new Error(`packaged desktop did not observe renderer loading under generated CSP\n${electronText}`);
    }
    if (!rendererAuthInjected) {
      throw new Error(`packaged desktop did not observe renderer authenticated request header injection\n${electronText}`);
    }
    if (!rendererSseOpened) {
      throw new Error(`packaged desktop did not observe renderer opening SSE connection\n${electronText}`);
    }

    const rowsBeforeShutdown = await processTable();
    const runtimePrefix = join(resources, "runtime");
    if (!rowsBeforeShutdown.some((row) => row.command.startsWith(runtimePrefix))) {
      throw new Error("packaged desktop runtime children exited during the health-stability window");
    }

    const launchedPid = launched.pid;
    if (launchedPid === undefined) throw new Error("packaged desktop has no process identifier");
    await requestNormalQuit(launchedPid);
    if (!await waitForExit(launched, SHUTDOWN_TIMEOUT_MS)) {
      throw new Error("packaged desktop did not honor the normal quit request within the shutdown timeout");
    }
    const cleanupStartedAt = Date.now();
    while (Date.now() - cleanupStartedAt < SHUTDOWN_TIMEOUT_MS) {
      const rows = await processTable();
      const childrenRemain = rows.some((row) => row.command.startsWith(runtimePrefix));
      let socketRemains = true;
      try {
        await lstat(kernelSocket);
      } catch (error: unknown) {
        if ((error as NodeJS.ErrnoException).code === "ENOENT") socketRemains = false;
        else throw error;
      }
    ...
    if (finalRows.some((row) => row.command.startsWith(runtimePrefix))) {
      throw new Error("packaged desktop left standalone runtime children running after shutdown");
    }
    try {
      await lstat(kernelSocket);
    ...
    logHandle = null;
    const observedArtifact = options.artifact === undefined
      ? undefined
      : await artifactEvidence(options.artifact);
    if (
      ...
      throw new Error("desktop ZIP artifact changed while its exact application was launched");
    }
    ...
    for (const digest of Object.values(evidence.application)) {
      if (!SHA256_PATTERN.test(digest)) throw new Error("desktop launch evidence contains an invalid digest");
    }
    completed = true;
    ...
    if (!completed) {
      if (child !== null) await terminateOwnedProcessTree(child).catch(() => undefined);
