...
- **macOS**: Seatbelt (`sandbox-exec`) backend translating `SandboxProfile` into deny-by-default Scheme syntax with per-subpath allowances (ADR-0035 §4); honestly reports `Enforced` when functional execution succeeds or `Degraded` when disabled/unsupported.
