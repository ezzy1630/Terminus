...
- **Linux**: Bubblewrap detection + real `bwrap` argv construction (`--unshare-all`, `--ro-bind`, `--proc`, `--dev`, `--die-with-parent`, `--new-session`, `--cap-drop ALL`). Reports `Enforced` when bwrap is available; reports `Unsupported` and refuses secure effect admission when it is unavailable.
- **macOS**: Seatbelt (`sandbox-exec`) backend translating `SandboxProfile` into deny-by-default Scheme syntax with per-subpath allowances (ADR-0035 §4); reports `Enforced` when functional execution succeeds or `Unsupported` and refuses secure effect admission when disabled/unsupported.
