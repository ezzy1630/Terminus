...
impl SqliteMetadataStore {
    ...
    fn lock(&self) -> Result<std::sync::MutexGuard<'_, Connection>, ArtifactError> {
        self.conn.lock().map_err(|e| ArtifactError::Sqlite(e.to_string()))
    }
}
...
mod tests {
    ...
    fn upsert_and_get_round_trip() {
        ...
        store
            .upsert(&meta, "/var/forge/artifacts/sha256/aa/a/aaa")
            .expect("upsert");
    ...
    fn link_inserts_row() {
        ...
        store
            .link(&hash, "task", "task-1", "input")
            .expect("link");
        assert_eq!(store.count_links().expect("count"), 1);
        let links = store
            .list_by_owner("task", "task-1")
            .expect("list");
