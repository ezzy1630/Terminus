...
pub struct DiagnoseResult {
    pub path: String,
    pub diagnostics: Vec<forge_kernel_protocol::Diagnostic>,
    pub message: String,
}
...
impl CodeIntelService {
    ...
    pub fn diagnose_files(
        &self,
        paths: &[String],
    ) -> Result<Vec<DiagnoseResult>, CodeIntelError> {
        // Placeholder: real diagnostics come from the language-fast validator
        // in forge-patch plus an LSP worker in a future crate.
        Ok(paths
            .iter()
    ...
    pub fn workspace_diff(
        &self,
        _paths: &[String],
    ) -> Result<WorkspaceDiff, CodeIntelError> {
        Ok(WorkspaceDiff {
            modified: Vec::new(),
            added: Vec::new(),
            removed: Vec::new(),
