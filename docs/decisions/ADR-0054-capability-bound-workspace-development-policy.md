...
1. Keep `secure-local-default` as the kernel's default command policy.
2. Add a named `workspace-development` command policy for native coding tasks.
   It admits arbitrary `EXECUTE_LOCAL` processes with bounded runtime, output,
   and environment, while hard-denying classified raw network access, external
...
3. Keep the enforced `secure-local-default` sandbox profile for both command
   policies. Ambient network and secrets remain unavailable; protected Git
   configuration and hooks remain write-protected. A command policy never
   selects or weakens a sandbox, and `workspace-development` is rejected if a
   caller asks for `degraded-local` or another sandbox profile.
4. Bind every non-default command policy to the signed task capability. The
...
5. Native standalone process tools request `workspace-development`; read,
   patch, verification, provider, scout, and external-harness paths keep their
   current profiles unless separately reviewed.
...
- Full workspace access becomes operational for ordinary local development
  without adding approval fatigue.
