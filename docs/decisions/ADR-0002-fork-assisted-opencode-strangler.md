...
4. Keep inherited behavior available behind feature flags until Forge-owned replacements reach parity.
5. Isolate Bun-specific APIs behind compatibility modules for Forge-owned code (ADR-0026).
...
- Forge-owned code MAY NOT use Bun-specific APIs except in the bridge (ADR-0026).
...
This ADR defines the migration itself. Each inherited package is replaced behind a Forge interface; the bypass register tracks the order.

## Rollback

If the strangler stalls (Risk R1), accelerate replacement of the affected package behind Forge interfaces. The exit strategy (SPEC §6.2) ensures the Rust boundary, ARP, Context IR, and evidence store survive any partial rollback.
