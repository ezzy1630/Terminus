...
This document specifies how Forge interprets research and vendor claims. The normative source is SPEC Appendix J.2; this document is a summary for contributors and decision-makers.
...
2. **Project README benchmark numbers are treated as project-reported until reproduced.** A coding-agent project claiming 70% on SWE-bench Verified is a hypothesis, not a fact, until Forge's eval lab reproduces it (or fails to).

3. **Vendor compression results are treated as vendor-reported until independently reproduced.** The Token Company's benchmarks (Appendix B) are vendor-reported. Forge's shadow-mode harness (ADR-0024) independently evaluates compression.
...
5. **Security papers demonstrate plausible attack classes; production controls are validated against concrete Forge threat fixtures.** A paper showing MCP tool poisoning is a warning; Forge's `evals/security/mcp-poisoning.yaml` is the validation.
...
| OpenCode plugins can auto-install packages and receive shell access in the upstream model | Secure Forge profile isolates installation and removes ambient plugin authority (ADR-0019). |
...
- **Interpretation note** — a short note explaining how the evidence maps to a Forge requirement.

This ensures that if a source changes or disappears, the original evidence is preserved.

## Applying research to Forge
...
3. State the interpretation: what does this evidence imply for Forge?
...
- Research does not replace Forge's eval lab. A paper showing X works on model M does not mean X works on Forge's cohorts with Forge's models.
