...
Revision: trusted candidate merge-receipt recovery commit `c56e245`; ledger state is recorded in `HANDOFF.md`.
...
HTTP/RPC route in mini-services/terminus-control/src/index.ts
  ...
  -> `ADMITTED` on durable admission via a validated trusted `EXECUTED` receipt (`reconcileAdmittingBranch`), `MANUAL_REVIEW` with a retained `NOT_EXECUTED`/`AMBIGUOUS` receipt, or `MANUAL_REVIEW` plus task `BLOCKED` after restart when no trusted receipt query is configured
