...
| C, context/provider | IN_PROGRESS | Safe cited compaction, full-window sizing, instruction loading, provider abort propagation, stream retry guards, durable provider-attempt identity, native response metadata, one live anonymous Zen free-model path, and revisioned kernel repository-map/native-recipe discovery are proven locally. Bounded continuation consumption is now complete-read validated; retrieval metrics/ablation, cache, monorepo-scale, paid-account, alternate-protocol, and broader conformance evidence remain open. |
...
| ACI-001 | IN_PROGRESS | `packages/aci`, control tools, kernel code intelligence | Bounded typed read/patch/exec/poll/search/fetch and revisioned repository-map RPCs remain kernel-mediated; complete map reads are continuation-validated and capped, while complete coding-ACI coverage, retrieval metrics, and monorepo-scale evidence remain open. |
...
| C4-01..C4-11 | IN_PROGRESS | Revisioned, source-attributed, scoped repository-map retrieval now follows and validates all bounded continuations before model projection; retrieval metrics/ablation, cache behavior, and monorepo-scale evidence remain open. |
...
| X-TEST-01..X-TEST-07 | IN_PROGRESS | Focused context/provider/lifecycle/security-adjacent tests, complete bounded repository-map continuation tests, native-recipe and signal-derived plan tests, an exact live anonymous Zen completion pass, and DB-backed effect/repair/proposal/cancellation/completion/coupled checkpoint-terminal/provider identity/provider-recovery/candidate-branch/verification replay tests pass; the deterministic lifecycle harness still has an unchanged 1,005/1,006 SSE overlap timeout, while trusted receipt reconciliation, alternate live paths, cross-platform, client, and governed UI tests remain. |
...
2. Add the governed UI predicate path to the verification deriver; keep unavailable UI capability explicit. Measure retrieval, cache, and continuation behavior around the new repository signals.
