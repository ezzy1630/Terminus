...
| B2-01..B2-11 | IN_PROGRESS | Provider/effect recovery classification, atomic `UNKNOWN`/`MANUAL_REVIEW` effect quarantine, safe boundary resume, fenced repair continuation recovery, completion admission replay, coupled checkpoint/terminal replay, cancellation, quarantine, and durable provider-attempt identity are wired; DB-backed effect/repair/completion/checkpoint/provider-identity scenarios pass, while trusted receipt reconciliation and complete fault-injection/replay proof remain open. |
...
| X-TEST-01..X-TEST-07 | IN_PROGRESS | Focused context/provider/lifecycle/security-adjacent tests, an exact live anonymous Zen completion pass, and DB-backed effect/repair/completion/coupled checkpoint-terminal/provider-identity replay tests pass; the remaining trusted receipt/proposal fault boundaries, alternate live paths, cross-platform, and client tests remain. |
...
2. Add DB-backed fault-injection/replay tests for proposal, branch admission, and cancellation boundaries; effect recovery, repair schedule/admission/fencing, completion admission, coupled checkpoint/terminal publication, and provider-attempt identity slices are now covered.
