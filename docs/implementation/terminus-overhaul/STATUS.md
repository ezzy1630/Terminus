...
| B, runtime correctness | IN_PROGRESS | The live loop now records proposal, verification, repair, finalization, and cancellation phases. Durable repair continuation, completion admission recovery, coupled checkpoint/terminal publication recovery, atomic ambiguous-effect quarantine, provider-attempt identity/recovery, atomic task cancellation, candidate-branch admission recovery with a trusted receipt-reconciliation contract, and exact verification resume are wired; full crash-resume coverage, a configured external receipt adapter, and state-owner extraction remain open. |
| C, context/provider | IN_PROGRESS | Safe cited compaction, full-window sizing, instruction loading, provider abort propagation, stream retry guards, durable provider-attempt identity, native response metadata, one live anonymous Zen free-model path, revisioned kernel repository-map/native-recipe discovery, versioned retrieval selection metrics with deterministic scoring ablations, and durable provider cache observation read-back are proven locally. Bounded continuation consumption is complete-read validated; a fresh eight-task cohort records average cache telemetry `0.7664` against the local `0.7` threshold with no warnings, while 20-run promotion, labeled retrieval outcomes, monorepo-scale, paid-account, alternate-protocol, and broader conformance evidence remain open. |
...
| B2-01..B2-11 | IN_PROGRESS | Provider/effect recovery classification, atomic `UNKNOWN`/`MANUAL_REVIEW` effect quarantine, no-duplicate in-flight provider recovery, safe boundary resume, exact verification resume, fenced repair continuation recovery, proposal quarantine, completion admission replay, coupled checkpoint/terminal replay, atomic cancellation, durable provider-attempt identity, candidate-branch `ADMITTING` -> `MANUAL_REVIEW` recovery, and the provider-neutral trusted merge-receipt contract (`reconcileAdmittingBranch`, migration `0019`) are wired; DB-backed effect/repair/proposal/cancellation/completion/checkpoint/provider/branch/receipt/verification scenarios pass, while a configured real external Git/merge receipt adapter, startup receipt-query wiring, and complete fault-injection/replay proof remain open. |
...
| C4-01..C4-11 | IN_PROGRESS | Revisioned, source-attributed, scoped repository-map retrieval follows and validates all bounded continuations before model projection; candidate features, additive score, exact cost, selection reason, versioned selection metrics, deterministic one-at-a-time ablations, and strict provider cache observation read-back are persisted. An eight-task fresh cache cohort satisfies the local average threshold; labeled outcome, no-learned-ranker promotion, 20-run release evidence, and monorepo-scale evidence remain open. |
| C5-01..C5-09 | IN_PROGRESS | Context IR and epochs exist; eight fresh sequential live tasks now have persisted predicted/realized cache observations with one cold and seven warm ratios above the local threshold on average, but 20-run release/evaluation promotion, stable-prefix/provider controls, and broader conformance remain open. Valid predicted/realized cached-token observations survive provider settlement and manifest read-back; malformed or absent values remain unavailable. |
...
| ID range | State | Notes |
| --- | --- | --- |
| X-DB-01..X-DB-08 | IN_PROGRESS | Existing SQLite migrations are monotonic; migrations `0012_repair_attempts`, `0013_completion_admission`, `0014_provider_attempt_identity`, `0015_candidate_branch_admission_recovery`, `0016_verification_recovery_identity`, `0017_provider_attempt_cost_accounting`, and `0018_provider_datetime_bigint`, and `0019_candidate_branch_merge_receipts` persist repair, completion-admission, provider-attempt identity/cost, branch recovery, verification resume, Prisma-compatible provider timestamps, and merge-receipt persistence, while the existing effect ledger records atomic recovery evidence without a new migration. DB-backed effect, repair, proposal/cancellation, completion, coupled checkpoint/terminal, provider-identity, candidate-branch, receipt, verification rollback/replay/fencing, and provider timestamp write/read coverage passes; journal completeness and other boundary coverage remain open. |
| X-OBS-01..X-OBS-08 | IN_PROGRESS | Semantic events, provider telemetry, and predicted/realized cache observations exist; an eight-task fresh live cache cohort is correlated through the kernel and durable database and the local average threshold is measured, while 20-run promotion, export, trusted receipt, and artifact-bound audits remain open. |
| X-TEST-01..X-TEST-07 | IN_PROGRESS | Focused context/provider/lifecycle/security-adjacent tests, complete bounded repository-map continuation tests, native-recipe and signal-derived plan tests, versioned retrieval metrics/scoring ablation tests, durable cache read-back tests, typed unavailable-UI blocking tests, an eight-task fresh live anonymous Zen cohort with one cold and seven warm cache observations, Prisma migration regression, and DB-backed effect/repair/proposal/cancellation/completion/coupled checkpoint-terminal/provider identity/provider-recovery/candidate-branch/trusted-receipt/verification replay tests pass; the deterministic lifecycle harness still has an unchanged 1,005/1,006 SSE overlap timeout, while 20-run/labeled outcome cohorts, broader cache promotion, real external receipt-adapter proof, alternate live paths, cross-platform, client, and actual governed UI tests remain. |
...
2. Wire a configured governed computer-use backend into the typed `ui_e2e` predicate and prove the real browser/desktop loop. Run the remaining 20-run release/evaluation cache cohort and labeled retrieval cohorts around the new repository signals.
3. Implement a trusted external Git/merge receipt adapter behind the provider-neutral boundary and wire startup recovery to a configured receipt query only after the adapter exists; the conservative `ADMITTING` -> `MANUAL_REVIEW` startup path and the `reconcileAdmittingBranch` receipt contract (c56e245, migration `0019`) are already committed.
