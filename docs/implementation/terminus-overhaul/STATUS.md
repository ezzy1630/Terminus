...
| ACI-001 | IN_PROGRESS | `packages/aci`, control tools, kernel code intelligence | Bounded typed read/patch/exec/poll/search/fetch and revisioned repository-map RPCs remain kernel-mediated; complete coding-ACI coverage, continuation consumption, and metrics remain open. |
...
| C4-01..C4-11 | IN_PROGRESS | Revisioned, source-attributed, scoped repository-map retrieval and first-page hydration are wired and tested; full retrieval metrics/ablation, cache behavior, continuation consumption, and monorepo-scale evidence remain open. |
...
| X-TEST-01..X-TEST-07 | IN_PROGRESS | Focused context/provider/lifecycle/security-adjacent tests, automatic repository-map/native-recipe and signal-derived plan tests, an exact live anonymous Zen completion pass, and DB-backed effect/repair/proposal/cancellation/completion/coupled checkpoint-terminal/provider identity/provider-recovery/candidate-branch/verification replay tests pass; trusted receipt reconciliation, alternate live paths, cross-platform, client, continuation, and governed UI tests remain. |
