...
| C4-01..C4-11 | IN_PROGRESS | Revisioned, source-attributed, scoped repository-map retrieval follows and validates all bounded continuations before model projection; candidate features, additive score, exact cost, selection reason, versioned selection metrics, deterministic one-at-a-time ablations, and strict provider cache observation read-back are persisted. Fresh live cache behavior, labeled outcome, no-learned-ranker promotion, and monorepo-scale evidence remain open. |
| C5-01..C5-09 | IN_PROGRESS | Context IR and epochs exist; stable-prefix/provider controls and fresh cache conformance need evidence. Valid predicted/realized cached-token observations now survive provider settlement and manifest read-back; malformed or absent values remain unavailable. |
...
| D2-01..D2-10 | IN_PROGRESS | `deriveVerificationNodes` now selects typed predicates from contract criteria, changed/scope paths, risk, instruction hashes, failures, diagnostics, generated paths, supplied native commands, and repository-map/native-recipe source signals. Admission and incremental modes have focused tests and a live caller; governed UI execution and complete semantic plan coverage remain open. |
...
| ID range | State | Notes |
| --- | --- | --- |
| X-DB-01..X-DB-08 | IN_PROGRESS | Existing SQLite migrations are monotonic; migrations `0012_repair_attempts`, `0013_completion_admission`, `0014_provider_attempt_identity`, `0015_candidate_branch_admission_recovery`, `0016_verification_recovery_identity`, and `0017_provider_attempt_cost_accounting` persist repair, completion-admission, provider-attempt identity/cost, branch recovery, and verification resume state, while the existing effect ledger records atomic recovery evidence without a new migration. DB-backed effect, repair, proposal/cancellation, completion, coupled checkpoint/terminal, provider-identity, candidate-branch, and verification rollback/replay/fencing coverage passes; journal completeness and other boundary coverage remain open. |
| X-OBS-01..X-OBS-08 | IN_PROGRESS | Semantic events, provider telemetry, and predicted/realized cache observations exist; fresh live cache correlation, export, threshold, and artifact-bound audits remain open. |
| X-TEST-01..X-TEST-07 | IN_PROGRESS | Focused context/provider/lifecycle/security-adjacent tests, complete bounded repository-map continuation tests, native-recipe and signal-derived plan tests, versioned retrieval metrics/scoring ablation tests, durable cache read-back tests, an exact live anonymous Zen completion pass, and DB-backed effect/repair/proposal/cancellation/completion/coupled checkpoint-terminal/provider identity/provider-recovery/candidate-branch/verification replay tests pass; the deterministic lifecycle harness still has an unchanged 1,005/1,006 SSE overlap timeout, while labeled outcome cohorts, fresh live cache behavior, trusted receipt reconciliation, alternate live paths, cross-platform, client, and governed UI tests remain. |
...
2. Add the governed UI predicate path to the verification deriver; keep unavailable UI capability explicit. Run labeled retrieval cohorts and fresh live cache observations around the new repository signals.
