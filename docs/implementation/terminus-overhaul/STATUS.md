...
| VER-001..VER-003 | IN_PROGRESS | verification/repair/index | Cumulative budget, durable failure signatures, source revision, automatic re-entry, durable attempt identity/leases, durable completion admission recovery, typed signal-derived plan selection, revisioned scoped repository-map retrieval, native-recipe source/version signals, and a fail-closed `ui_e2e` predicate boundary are wired; actual governed UI execution, repair metrics, and full replay proof remain open. |
...
| X-TEST-01..X-TEST-07 | IN_PROGRESS | Focused context/provider/lifecycle/security-adjacent tests, complete bounded repository-map continuation tests, native-recipe and signal-derived plan tests, versioned retrieval metrics/scoring ablation tests, durable cache read-back tests, typed unavailable-UI blocking tests, an exact live anonymous Zen completion pass, and DB-backed effect/repair/proposal/cancellation/completion/coupled checkpoint-terminal/provider identity/provider-recovery/candidate-branch/verification replay tests pass; the deterministic lifecycle harness still has an unchanged 1,005/1,006 SSE overlap timeout, while labeled outcome cohorts, fresh live cache behavior, trusted receipt reconciliation, alternate live paths, cross-platform, client, and actual governed UI tests remain. |
...
2. Wire a configured governed computer-use backend into the typed `ui_e2e` predicate and prove the real browser/desktop loop. Run labeled retrieval cohorts and fresh live cache observations around the new repository signals.
