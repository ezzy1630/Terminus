...
| C, context/provider | IN_PROGRESS | Safe cited compaction, full-window sizing, instruction loading, provider abort propagation, stream retry guards, durable provider-attempt identity, native response metadata, one live anonymous Zen free-model path, revisioned kernel repository-map/native-recipe discovery, and versioned retrieval selection metrics with deterministic scoring ablations are proven locally. Bounded continuation consumption is complete-read validated; labeled retrieval outcomes, cache, monorepo-scale, paid-account, alternate-protocol, and broader conformance evidence remain open. |
...
| X-TEST-01..X-TEST-07 | IN_PROGRESS | Focused context/provider/lifecycle/security-adjacent tests, complete bounded repository-map continuation tests, native-recipe and signal-derived plan tests, versioned retrieval metrics/scoring ablation tests, an exact live anonymous Zen completion pass, and DB-backed effect/repair/proposal/cancellation/completion/coupled checkpoint-terminal/provider identity/provider-recovery/candidate-branch/verification replay tests pass; the deterministic lifecycle harness still has an unchanged 1,005/1,006 SSE overlap timeout, while labeled outcome cohorts, trusted receipt reconciliation, alternate live paths, cross-platform, client, and governed UI tests remain. |
...
2. Add the governed UI predicate path to the verification deriver; keep unavailable UI capability explicit. Run labeled retrieval cohorts and measure cache and continuation behavior around the new repository signals.
