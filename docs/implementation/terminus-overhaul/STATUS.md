...
| D3-01..D3-08, D3-M01 | IN_PROGRESS | One task-level budget, durable signatures including evidence identity, typed directive, durable attempt/lease identity, automatic child re-entry, parent supersession, re-verification, and DB-backed fencing replay are wired. Replayable repair metrics are derived in `@terminus/verification` and exposed on `GET /v1/tasks/:id`; trusted provider cost and live aggregate collection remain open. |
