...
29. Migration `0018_provider_datetime_bigint.sql` was applied to a fresh isolated control database after the live `PUT /v1/gateway-provider-config` exposed the `INTEGER` overflow. `tests/persistence/migration_integrity.test.ts` now applies all 18 migrations and uses the generated Prisma client to create and read both provider rows with millisecond timestamps beyond the 32-bit range. The focused suite passed 5/5 tests; `just codegen-check` includes the new migration in generated inventory/SQLx documentation.
...
This closes the live-provider proof for one supported anonymous public Zen path and records one fresh cache observation. It does not close paid-account, alternate-protocol, broader cache-cohort, retrieval-outcome, cross-platform, hosted-CI, or release gates.
