...
| HEAD at last implementation evidence capture | `6850036` (`Wire revisioned repository maps and native recipe discovery`) |
| Branch | `main` |
| Remote state at last implementation evidence capture | Forty commits ahead of `origin/main`; no push performed |
| Functional worktree | Functional paths are clean at `6850036`; `SPEC.md` retains a pre-existing user edit |
...
25. `CodeIntelligenceService.Map` now returns a deterministic source-only file-to-symbol map with a canonical index revision, total count, and opaque continuation. The kernel applies workspace path scope before pagination and rejects a stale continuation revision. The control plane validates paths, symbols, hashes, counts, and truncation, injects a source-attributed first-page map fragment into retrieval, and passes map provenance into verification-plan derivation. This proves the bounded local contract and scoped/stale rejection tests; it does not prove full continuation consumption, retrieval ablation, or a live end-to-end execution of every discovered recipe.
