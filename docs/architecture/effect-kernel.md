...
|---|---|---|
| `KernelInfoService` | Version, health, supported backends/services | `forge-kernel` |
| `FileService` | Read files with outlines, ranges, symbols, hashes, elisions, artifacts | `forge-fs` |
| `PatchService` | Apply and reconcile journaled patch transactions | `forge-patch` |
| `ProcessService` | Start structured processes; stream stdout/stderr/exited | `forge-process` |
| `JobService` | Durable, restart-survivable jobs; stream, input, signal, stop, get | `forge-jobs` |
| SandboxService | Construct/teardown sandboxes; report capabilities | `forge-sandbox` |
| PolicyService | Evaluate effect policy; return decision | `forge-policy` |
| SecretService | Issue/revoke capabilities; inject at exec; redact output | `forge-secrets` |
| NetworkService | Egress proxy; destination allowlist; DNS brokering | `forge-egress` |
| GitService | Protected worktree/commit/merge operations | `forge-git` |
| CodeIntelService | Tree-sitter symbols, structural search, LSP/DAP facade | `forge-code-intel` |
| ExtensionService | WASI/process extension host; capability enforcement | `forge-extension-runtime` |
| ArtifactService | Content-addressed ingest, streaming, GC | `forge-artifacts` |
| AuthService | Kernel instance identity; short-lived capability tokens | `forge-authz` |

The full protocol is in `proto/forge/kernel/v1/kernel.proto` (Appendix D). The mini-service bootstrap uses JSON-over-HTTP; the canonical protocol is gRPC over UDS.
...
4. **Policy decision** — `forge-policy` evaluates the effect against the active policy profile.
5. **Approval check** — if required, verify an approval record bound to the normalized action hash.
6. **Sandbox enforcement** — `forge-sandbox` constructs/reuses a sandbox for the operation.
...
Tokens are validated on every RPC. Compromised tokens are revocable. The `forge-authz` crate implements token issuance and validation.

## Sandbox backends (SPEC §13.4, §36.5–§36.8, ADR-0014, ADR-0027)

| Backend | Platform | Status | Use case |
|---|---|---|---|
| `forge-sandbox-linux` (Bubblewrap) | Linux | ADOPTED | Default local trusted workspace |
| `forge-sandbox-macos` | macOS | Scaffolded | macOS client/control (honest degraded reporting) |
| `forge-sandbox-windows` | Windows | Scaffolded | Windows client/control (honest degraded reporting) |
| `forge-sandbox-container` | Linux | OPEN (ADR-0027) | Untrusted repos, evals, extensions |
...
Degraded-mode detection (SPEC §36.4, §26.3 #11): when the requested sandbox cannot be enforced, Forge fails closed or requires explicit user selection of a named degraded profile.

## Command policy (SPEC §13.5, §36.9)

The command policy engine (`forge-policy`) normalizes commands via a shell AST parser and evaluates them against versioned rule sets. The default rule set (`policies/command/default.yaml`) has 6 rules: allow-local-tests, allow-read-tools, prompt-git-push, deny-download-pipe-interpreter, deny-protected-path-write, deny-external-state-write-default.
...
- Output redaction (`forge-secrets/src/redact.rs`).
...
Implementation: `crates/forge-egress`. Default network policy: `policies/network/default.yaml`.
...
Implementation: `crates/forge-patch` + `crates/forge-fs`.
...
Implementation: `crates/forge-process` + `crates/forge-jobs`.
...
Implementation: `crates/forge-extension-runtime`.

## Code intelligence (SPEC §11.8, §34.13)

- Tree-sitter symbols and structural search (`crates/forge-code-intel/src/symbols.rs`, `inspect.rs`).
- LSP/DAP facade.
