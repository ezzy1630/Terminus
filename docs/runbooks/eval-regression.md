...
Use this runbook when the eval lab detects a regression: a feature, configuration, or upstream sync causes a cohort's success rate, cost, or safety sub-metric to regress beyond the non-regression threshold (SPEC §46.18, §50.9). Eval regressions block releases.
...
2. **If the regression is on the primary metric (verified success/cost):**
   ...
   - For an upstream sync: consider reverting the sync or fixing the regression before re-syncing.
...
- Add a parity test if the regression was caused by an upstream sync.
...
- `docs/runbooks/upstream-merge-conflict.md` — if the regression was caused by an upstream sync.
