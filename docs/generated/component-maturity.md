...
| `aci` | ts-package | `packages/aci` | `experimental` | Agent-Computer Interface tools; standalone surface carries the R1 read/patch source-hash contract (numbered reads, observed-hash patch resolution) with unit tests; live-model conformance unproven at HEAD |
