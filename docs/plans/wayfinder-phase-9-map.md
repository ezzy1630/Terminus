...
- **OrganizationDirectory**: federated organization topology, departmental workspaces, persistent operator agents, agent rooms, and deterministic capability resolution;
...
- **CausalReplayEngine**: step lineage recording, deterministic replay of non-model steps, pinned-input model re-execution, context omission diagnostics, and counterfactual simulation;
- **DesktopCockpit**: 10-view operator cockpit (`OrganizationMapView`, `DepartmentRoomsView`, `MissionLedgerView`, `WorkflowGraphView`, `WorldStateView`, `EffectQueueView`, `ClaimEvidenceGraphView`, `ArtifactDiffInspectorView`, `FleetBudgetView`, `CausalReplayView`, `AttentionCenterModal`, `StructuredInterventionModal`);
- **AcpBridge**: Agent Client Protocol JSON-RPC adapter for editor synchronization (workspace selection, active file, diagnostics, open tabs, inline approvals).
...
- Organization and Capability Directory (`OrganizationDirectory`) providing deterministic capability resolution without a centralized root agent bottleneck.
...
- Causal Replay Engine (`CausalReplayEngine`) recording lineage and supporting
  omission diagnostics only with trusted evaluator evidence; counterfactual
  outputs remain hypotheses.
...
- Task-first CLI commands in `apps/cli`, text-mode TUI commands in `apps/tui`,
  and ACP-over-stdio methods in `apps/ide-acp`.
- Desktop operator cockpit routes and modal overlays in `apps/desktop`.
